...
If Hour(Time) = vapo_h And Minute(Time) = vapo_m Then
    ...
    Wrap$ = Chr$(13) + Chr$(10)
    Text10.Text = Text10.Text & Wrap & STARTPROCEDURE_MAIL
    Label8.Visible = True
End If
...
'****************************** AVI CHECK  **********************************8
    ...
    If Hour(Time) = vmexri_h And Minute(Time) = vmexri_m Then
        ...
        End If
        Wrap$ = Chr$(13) + Chr$(10)
        Text10.Text = Text10.Text & Wrap & AVI_EXIST_MAIL & Wrap & AVI_SIZE_MAIL
...
FILES_PATH_DEFAULT = "C:\HELIX\OUTPUT\" & Day(Date) & "-" & Month(Date) & "-" & Year(Date)
...
If Hour(Time) = vapo_h And Minute(Time) = vapo_m Then
    If FLAG_INCLUDE_RM = 0 Then
        Call Shell("C:\CARTOON\START_NORM.bat", vbNormalFocus) 'IMPORTANT
    Else
        Call Shell("C:\CARTOON\START_RM.bat", vbNormalFocus) 'IMPORTANT
    End If
    FLAGSTARTPROCEDURE = 1
    STARTPROCEDURE_MAIL = "г диадийасиа нейимгсе йамомийа~~~"
    Text31.Text = STARTPROCEDURE_MAIL
...
If SENDINITIALMAILNOW = 1 Then
    'Text9.Text = ""
    'Text9.Text = "cd C:\CARTOON\HELIX\sendEmail-v155"
    'Wrap$ = Chr$(13) + Chr$(10)
    'Text9.Text = Text9.Text & Wrap & _
    '"sendEmail.exe -f CARTOON@velti.com -t nlazarou@velti.com -u " & _
    'INITIAL_MESSAGE_MAIL_CAPTION & " -m " & INITIAL_MESSAGE_MAIL & " -s hermes:25"
    'CommonDialog2.FileName = "C:\HELIX\CARTOON\MAILS\initial.bat"
    'Open CommonDialog2.FileName For Output As #1
    'Print #1, Text9.Text
    'Close #1
    ' SEND TO INITIAL
    'Call Shell("C:\HELIX\CARTOON\MAILS\initial.BAT", vbNormalFocus)
...
If PREPARE_MAIL = 1 Then
    ...
    Else
        If (FLAG_AVI = 3) Or (FLAG_263GP = 3) Or (FLAG_823GP = 3) Or (FLAG_25RM = 3) Or (FLAG_82RM = 3) Then
            MAIL_CAPTION = "WARING ~~~~ сто CARTOON йапоио(а) аявеио(а) жаиметаи ма лгм свглатистгйе сыста."
           Text30.Text = MAIL_CAPTION
            ARXIKOPOIHSH
            SENDMAILNOW = 1
            FLAGSTARTPROCEDURE = 0
...
If SENDMAILNOW = 1 Then
    'SEND TO MAIL ME CAPTION TEXT30 KAI BODY TEXT10
    '******************** MAIL ************************
    Call Shell("C:\SENDMAIL\sendEmail.exe -f CARTOON@velti.com -t nlazarou@velti.com -u " & Text30.Text & " -m " & Text10.Text & " -a C:\HELIX\C:\HELIX\LOGS\FTP_LOGS\FTPLOG_" & Date & ".TXT -s hermes:25 ", vbNormalFocus)
    ARXIKOPOIHSH
    SENDMAILNOW = 0
    '****************** grapsimo logs ****************************************8
