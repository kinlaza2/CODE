...
NNIK:
    Do While Not rsp.EOF
        If rsp![омола_етаияиас] = Text1.Text Then
            D = D + 1
            rsp.MoveNext
        Else
            rsp.MoveNext
...
If MsgBox("хекете ма пяовыягсете стгм еццяажг тгс пяосжояас", vbOKCancel, "") = vbOK Then
    ...
    Else
       MsgBox ("циа то пяоиом : " & Text5.Text & "  упаявеи гдг пяосжояа апо тгм етаияиа :  " & Text1.Text & " паяайакы екецнте"), vbCritical, "пяосовг !!!"
       GoTo TELOS:
    End If
    ...
    End If
    '************************************ PERASMA DEDOMENON SE PINAKA ETAIRION******
    '*******************************************************************************
    ...
    ' ANIXNEYSH AN YPARXEI ETAIRIA KAI AN OXI DHMIOYRGIA
    If rsp.EOF = rsp.BOF Then GoTo HH:
    rsp.MoveFirst
HH:
    Do While Not rsp.EOF
        If Text1.Text = rsp![етаияиес] Then
            count = count + 1
            rsp.MoveNext
        Else
            rsp.MoveNext
    ...
    If count = 0 Then
        SSS = " create table " & Text1.Text & "(" & _
            "омола_пяоиомтос varchar(50) ," & _
            "тилг FLOAT ," & _
            "ейптысг FLOAT ," & _
            "сглеиысг varchar(250)," & _
            "PRIMARY KEY (омола_пяоиомтос) )"
        dbp.Execute SSS
        SSS2 = "INSERT INTO ETAIRIES_ABC (" & _
        "етаияиес) VALUES (" & _
        "'" & UCase(Text1.Text) & "')"
        dbp.Execute SSS2
    End If
...
TEXT1KENO:
    ...
    GoTo TELOS:
TEXT1BIGLEN:
    MsgBox ("то омола тгс етаияиас дем пяепеи ма еимаи лецакутеяо апо 50 ваяайтгяес"), vbCritical, "пяосовг !!!"
    Text1.SetFocus
    GoTo TELOS:
TEXT1WCHAR:
    MsgBox ("то омола етаияиас дем пяепеи ма пеяиевеи тоус ваяайтгяес [ ] . !"), vbCritical, "пяосовг !!!"
    Text1.SetFocus
    GoTo TELOS:
TEXT2KENO:
...
NNIK:
    Do While Not rsp.EOF
        If rsp![омола_етаияиас] = Text1.Text Then
            D = D + 1
            Text7.Text = rsp![омола_етаияиас]
            Text1.Text = rsp![омола_етаияиас]
            Text2.Text = rsp![тилг]
...
Else        ' ******************************* AN DIORTHOSI ********************
    ...
    Dim ERT As Integer
    ERT = 0
    If Text1.Text = Text7.Text Then ' AN TO ONOMA ETAIRIAS DEN ALLAZEI
    
    Else ' AN TO ONOMA ETAIRIAS ALLLAZEI
        If dbp.STATE = 1 Then dbp.Close
        dbp.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
        "Data Source=" & "\databases\PROION.mdb" & ";" & _
        "Persist Security Info=False"
...
NIKKK:
    ...
    If MsgBox("хекете ма пяовыягсете стгм диояхысг тгс еццяажгс", vbOKCancel, "") = vbOK Then
        STATE1 = " UPDATE " & Text5.Text & _
                ...
                " WHERE омола_етаияиас=" & "'" & Text7.Text & "'"
        ...
        STATE4 = " UPDATE " & Text5.Text & _
                " SET омола_етаияиас=" & "'" & Text1.Text & "'" & _
                " WHERE омола_етаияиас=" & "'" & Text7.Text & "'"
        dbp.Execute STATE1
        dbp.Execute STATE2
        dbp.Execute STATE3
...
'***************************************************************************************
        ...
        ' AN DEN EXEI ALLAXTEI TO ONOMA ETAIRIAS
        If Text1.Text = Text7.Text Then
                SSS1 = " UPDATE " & Text7.Text & _
                " SET тилг=" & "'" & Text2.Text & "'" & _
                " WHERE омола_пяоиомтос=" & "'" & Text5.Text & "'"
...
EE:
            Do While Not rsp.EOF
                If rsp![етаияиес] = Text1.Text Then
                    CCOUNT = CCOUNT + 1
                    rsp.MoveNext
                Else
                    rsp.MoveNext
            ...
            If CCOUNT = 0 Then 'TO NEO ONOMA DEN YPARXEI HDH
                ...
                DS = "DELETE FROM " & Text7.Text & " WHERE омола_пяоиомтос='" & _
                 Text5.Text & "'"
                dbp.Execute DS
                ...
                'AN O PINAKAS POY APOMENEI EINAI ADEIOS SBHSTON
                    ...
                    d_file = App.Path & "\databases\PROION_ETAIRIA.mdb"
                    ...
                    If Text8.Text = 0 Then
                        ...
                        s22 = " DELETE FROM ETAIRIES_ABC " & _
                                " where етаияиес= '" & Text7.Text & "'"
                        dbp.Execute s11
                        dbp.Execute s22
                    End If
...
NN1:
                Do While Not r1.EOF
                    If r1![омола_пяоиомтос] = Text5.Text Then
                        r1.MoveNext
                    Else
                        KK = "DELETE FROM " & Text1.Text & " WHERE " & _
                        " омола_пяоиомтос='" & r1![омола_пяоиомтос] & "'"
                        dbp.Execute KK
                        r1.MoveNext
                    End If
                Loop
                
                US = "INSERT INTO ETAIRIES_ABC (" & _
                "етаияиес) VALUES (" & _
                "'" & UCase(Text1.Text) & "')"
                dbp.Execute US
...
eee:
                Do While Not rrsp.EOF
                    If rrsp![омола_пяоиомтос] = Text5.Text Then
                        c2 = c2 + 1
                        rrsp.MoveNext
                    Else
                        rrsp.MoveNext
                ...
                If c2 = 0 Then
                    ...
                    "'" & UCase(Text4.Text) & "'" & _
                    ")"
                    ctate2 = "delete from " & Text7.Text & " where омола_пяоиомтос='" & Text5.Text & "'"
                    dbp.Execute ctate
                    dbp.Execute ctate2
...
TEXT4BIGLEN:
    ...
    GoTo TELOS:
norecord:
 MsgBox ("дем бяехгйе еццяажг ле то омола етаияиас поу дысате"), vbCritical, "пяосовг !!!"
    Text4.SetFocus
    GoTo TELOS:


YPAR_ETAIR:
MsgBox ("то омола етаияиас поу пяоспахгте ма дысете упаявеи гдг"), vbCritical, "пяосовг !!!"
GoTo TELOS:


QWERTY:
MsgBox ("то мео омола етаияиас поу дысате упаявеи гдг йаи евеи йатавыяглемо то пяоиом : " & Text5.Text), vbCritical, "пяосовг !!!"
GoTo TELOS:
...
Else  '***********************  PROGRAMMATISMOS *******************************
   If MsgBox("хекете ма пяовыягсете стгм диацяажг тгс еццяажгс", vbOKCancel, "пяосовг") = vbOK Then
            STATEMENT = " delete from " & Text5.Text & _
                        " where омола_етаияиас= '" & Text1.Text & "'"
            dbp.Execute STATEMENT
            'RSH.Fields.Refresh
            'RSH.Close
            If dbp.STATE = 1 Then dbp.Close
...
'*********************************************************************************
        ...
        dbp.Execute SSS
         ...
         If Text8.Text = 1 Then
            ...
            SSS2 = " DELETE FROM ETAIRIES_ABC " & _
            " where етаияиес= '" & Text1.Text & "'"
            dbp.Execute SSS1
            dbp.Execute SSS2
         End If
'*********************************************************************************
...
Text5.Text = proionta1.Text1.Text
Label5.Caption = "пяоиом :" & Text5.Text
